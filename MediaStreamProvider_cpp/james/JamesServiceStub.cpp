#include "JamesServiceStub.h"
#include <string>
#include "JamesServiceConfig.h"
static IJamesService *spIService;
...
//dbus-send --system --type=method_call --print-reply --dest=fxn.msproviderservice /fxn/msproviderservice fxn.msproviderservice.cmd1 string:'{"key":"audio_speaker_volume"}'
static const gchar* introspection_xml = {
    "<node>"
    "  <interface name='fxn.msproviderservice'>"
    "    <method name='cmd1'>"
...
#include "MLogger.h"
void
JamesServiceStub::handle_method_call(GDBusConnection *connection,
                                 const gchar *sender,
    ...
    if (g_strcmp0(interface_name, INTERFACE_NAME) == 0) {
        if (g_strcmp0(method_name, "cmd1") == 0) {
            ...
            (void) spIService->cmd1(pdata, result);
            //result = "ServiceA::cmd1";
        } else if (g_strcmp0(method_name, "cmd2") == 0) {
            g_variant_get(parameters, "(&s)", &pdata);
            (void) spIService->cmd2(pdata, result);
...
GVariant *
JamesServiceStub::handle_get_property(GDBusConnection *connection,
    ...
    if (g_strcmp0(interface_name, INTERFACE_NAME) == 0) {
        if (g_strcmp0(property_name, "Version") == 0) {
            std::string str;
            (void) spIService->GetAPIVersion(str);
...
gboolean JamesServiceStub::handle_set_property(GDBusConnection *connection, const gchar *sender,
...
//==============================================
JamesServiceStub::JamesServiceStub(IJamesService *pservice) {
    if (pservice == nullptr) {
        //MLogger::LOG(Level::ERROR, "ServiceAStub: Service is null");
    } else {
        spIService = pservice;
        spIService->SetListener(this);
    }
}
...
bool JamesServiceStub::IsReady() {
    return s_ready;
}

int JamesServiceStub::onJamesServiceCallback(const std::string &str) {
    int v = 0;
